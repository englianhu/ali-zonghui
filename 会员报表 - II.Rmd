...
pkgs <- c('plyr', 'dplyr', 'magrittr', 'stringr', 'knitr', 'kableExtra', 'lubridate', 'broom', 'purrr', 'readxl', 'tibble', 'DT', 'formattable', 'conflicted')
...
```{r warning = FALSE}
## 读取数据
fls <- suppressWarnings(list.files('诸子百家学府/会员报表'))
smp <- fls %>% llply(., function(x) {
    dtt <- x %>% str_replace_all('.xls', '') %>% ymd
    smpp <- read_excel(paste0('诸子百家学府/会员报表/', x))
