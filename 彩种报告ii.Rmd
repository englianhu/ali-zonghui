...
<span style='color:Orange'>*方程3.1：线型模型*</span>
...
<span style='color:Orange'>*方程3.1.1：最优模型AIC比较*</span>
...
<span style='color:Orange'>*方程3.1.2：最优模型AIC比较*</span>
...
<span style='color:Orange'>*方程3.1.2.1：线型模型*</span>
...
```{r}
dir('data', pattern = 'mp') %>% llply(., function(x) {
    read_rds(paste0('data/', x)) %>% 
        mutate(.md = str_replace_all(x, '.rds', '')) %>% 
        dplyr::select(.md, .id, df, AIC)
}) %>% ldply %>% 
    as_tibble %>% 
    pack_rows('Group 1', 1, 22, label_row_css = 'background-color: #666; color: #fff;')
```
